...
# GUIで表示する。
PS> Invoke-WebRequest "https://mostodon.cloud/api/v1/timelines/public?local=1&limit=4" | %{ $_.Content } | ConvertFrom-Json |  %{ $names = $_.account.username; $conts = $_.content; $dt=$_.created_at; 0..($names.Length-1) | %{ [pscustomobject]@{name=$names[$_]; contents=$conts[$_]; date=$dt[$_]}  } | Out-GridView }
```
